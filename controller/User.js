...
//         res.status(200).json({
//             success: true,
//             message: "Added To Travel & Photography",
//         });
//     } catch (error) {
//         return res.status(400).json({
//             success: false,
...
export const addPhoto = async (req, res) => {
    try {
        ...
        const myCloud = await cloudinary.v2.uploader.upload(image, {
            folder: "portfolio/travel",
        });

        // Push into travel array
        user.travel.unshift({
            title,
            image: {
                public_id: myCloud.public_id,
...
//         const photo = user.travel.find((photo) => photo._id == id);

//         await cloudinary.v2.uploader.destroy(photo.image.public_id);

//         user.travel = user.travel.filter((photo) => photo._id != id);

//         await user.save();

//         res.status(200).json({
//             success: true,
//             message: "Deleted from Travel & Photography.",
//         });
//     } catch (error) {
//         return res.status(400).json({
//             success: false,
...
export const deletePhoto = async (req, res) => {
    try {
        ...
        // Find photo in travel array
        const photo = user.travel.find((p) => p._id.toString() === id);
        if (!photo) {
            return res.status(404).json({
                success: false,
        ...
        // Remove from array
        user.travel = user.travel.filter((p) => p._id.toString() !== id);
